...
static const ArrayIteratorClass ArrayIteratorDescription = {
    {
        {
...
static const Class *ArrayIterator = (const Class *)&ArrayIteratorDescription;

static void Array_ctor(ArrayClass *this, va_list *args)
{
    size_t array_size = va_arg((*args), size_t);
    Class *class_type = va_arg((*args), Class*);

    this->_tab = malloc(sizeof(Object *) * array_size);
    if (!this->_tab)
        raise("Out of memory");
    for (unsigned int i = 0; i < array_size; i++) {
        this->_tab[i] = malloc(sizeof(Object) * class_type->__size__);
        this->_tab[i] = va_new(class_type, args);
    }
}
